...
<table border="1" width="100%">
	<tr style="background:#eeeeee">
						...
						if(String.valueOf(account.getUserId()).equalsIgnoreCase(supervisor.getSupervisorId().toString())){
							if(!supervisor.getType().equalsIgnoreCase("teacher")){
								...
								while(sectionItr.hasNext()){
									...
									if(41<=s.getId() && s.getId()<=46 && Integer.parseInt(supervisor.getLevelfromId())<=s.getLevelId() && s.getLevelId()<=Integer.parseInt(supervisor.getLeveltoId())){
										%><option value="<%=s.getId()%>"><%=s.getSectionName()%></option><%
									}
								}
							}else{
								if( 41<=Integer.parseInt(supervisor.getSectionId()) && Integer.parseInt(supervisor.getSectionId())<=46){
		...
		<td>
			<input type="button" value="Show" onclick="showListOfStudentsInThisSectionForIB(document.getElementById('slctsection').value);"/>
		</td>
	</tr>
</table>
<div id="displayReportContentDivIB"></div>