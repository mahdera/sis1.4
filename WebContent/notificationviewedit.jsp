...
<%
	...
	attendance = AttendanceNotification.getAttendanceForEdit(teacherId,
	...
	<form>
		<table id="attendance" width="100%">
			...
			<tr width="100" style="background: #fff">
				...
				<td><%=a.getStudId()%></td>
				<%if(!type.equalsIgnoreCase("Absent")) {%>
				<%countLate=AttendanceNotification.getLateCountForEdit(sectionId, level, attendanceSession, dateFrom, dateTo,ids.get(id_counter));%>
				<td><%=countLate%></td>
				<%datesLate=AttendanceNotification.getLateDatesForEdit(teacherId,sectionId, level, attendanceSession, dateFrom, dateTo,ids.get(id_counter));%>
				<td><%=datesLate%></td>
				<%} 
				if(!type.equalsIgnoreCase("Late")) {
				%>
				<%countAbsent=AttendanceNotification.getAbsentCountForEdit(sectionId, level, attendanceSession, dateFrom, dateTo,ids.get(id_counter));%>
				<td><%=countAbsent%></td>
				<%datesAbsent=AttendanceNotification.getAbsentDatesForEdit(teacherId,sectionId, level, attendanceSession, dateFrom, dateTo,ids.get(id_counter));%>
				<td><%=datesAbsent%></td>
				<%} %>
				<td><font color="green"><i><input type="text" name="txtreason"
					style="height: 50px; width: 100%;" id="txtreason<%=rowc%>" size="10" value="<%=a.getReason()%>" /></i></font></td>
				<td><button onclick="savenotifaction(document.getElementById('txtreason<%=rowc%>').value,
			    '<%=attendance.get(rowc).getStudId()%>','<%=sectionId%>','<%=attendanceSession%>','<%=type%>','N');return false;">dealt</button>
				</td>
			</tr>
			<%
				counter++;
