		...
		while(earlyYearsEndYearEvaluationItr.hasNext()){
			...
			%>
				<tr>
					...
					<td>
						<a href="#.jsp" onclick="showReceptionEndYearReportDetailEditResult(<%=earlyYearsEndYearEvaluation.getId() %>);" class="showReportLinkClass">Edit Detail</a>
						|
						<a href="#.jsp" onclick="hideDiv(<%=earlyYearsEndYearEvaluation.getId() %>)" class="hideReportLinkClass">Hide Detail</a>
					</td>
				</tr>
				<%
					String divId = "receptionEndYearReportDiv" + earlyYearsEndYearEvaluation.getId();
				%>
				<tr>
					<td colspan="7">
						<div id="<%=divId %>"></div>
...
<script type="text/javascript">
	function showReceptionEndYearReportDetailEditResult(id){
		var divId = "receptionEndYearReportDiv" + id;
		$('#'+divId).load("showreceptionendyearreportdetaileditresult.jsp?id="+id);
	}
