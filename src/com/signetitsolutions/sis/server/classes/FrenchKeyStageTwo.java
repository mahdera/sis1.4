 ...
 *
 */
public class FrenchKeyStageTwo {
	private long id;
	 ...
	 */
	public FrenchKeyStageTwo(int sectionId, String studentIdNumber,
	 ...
	 * @param modificationDate
	 */
	public FrenchKeyStageTwo(long id, int sectionId, String studentIdNumber,
			int teacherId, String speakingAndListeningAchievement,
			int speakingAndListeningEffort, String readingAchievement,
			int readingEffort, String writingAchievement, int writingEffort,
			String classTeacherComments, Date dateCreated, int modifiedBy,
	...
	public void save() {
		try {
			String sqlStr = "insert into tbl_french_key_stage_two values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static void update(FrenchKeyStageTwo frenchKeyStageTwo) {
		try {
			String sqlStr = "update tbl_french_key_stage_two set section_id = ?, student_id_number = ?," +
			...
			PreparedStatement pStmt = DBConnection.getPreparedStatement(sqlStr);
			pStmt.setInt(1, frenchKeyStageTwo.getSectionId());
			pStmt.setString(2, frenchKeyStageTwo.getStudentIdNumber());
			pStmt.setInt(3, frenchKeyStageTwo.getTeacherId());
			pStmt.setString(4, frenchKeyStageTwo.getSpeakingAndListeningAchievement());
			pStmt.setInt(5, frenchKeyStageTwo.getSpeakingAndListeningEffort());
			pStmt.setString(6, frenchKeyStageTwo.getReadingAchievement());
			pStmt.setInt(7, frenchKeyStageTwo.getReadingEffort());
			pStmt.setString(8, frenchKeyStageTwo.getWritingAchievement());
			pStmt.setInt(9, frenchKeyStageTwo.getWritingEffort());
			pStmt.setString(10, frenchKeyStageTwo.getClassTeacherComments());
			pStmt.setInt(11, frenchKeyStageTwo.getModifiedBy());
			pStmt.setDate(12, frenchKeyStageTwo.getModificationDate());
			pStmt.setLong(13, frenchKeyStageTwo.getId());
			DBConnection.writeToDatabase(pStmt);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void delete(long id) {
		try {
			String sqlStr = "delete from tbl_french_key_stage_two where id = ?";
			PreparedStatement pStmt = DBConnection.getPreparedStatement(sqlStr);
			pStmt.setLong(1, id);
			DBConnection.writeToDatabase(pStmt);
		} catch (Exception e) {
	...
	public static List<FrenchKeyStageTwo> getAllFrenchKeyStageTwos() {
		List<FrenchKeyStageTwo> list = new ArrayList<FrenchKeyStageTwo>();
		FrenchKeyStageTwo frenchKeyStageTwo = null;
		try {
			String query = "select * from tbl_french_key_stage_two order by modification_date desc";
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				frenchKeyStageTwo = new FrenchKeyStageTwo(rSet.getLong("id"), rSet.getInt("section_id"), 
						rSet.getString("student_id_number"), rSet.getInt("teacher_id"), 
								rSet.getString("speaking_and_listening_achievement"), 
								rSet.getInt("speaking_and_listening_effort"), rSet.getString("reading_achievement"), 
								rSet.getInt("reading_effort"), rSet.getString("writing_achievement"), 
								rSet.getInt("writing_effort"), rSet.getString("class_teacher_comments"), 
								rSet.getDate("date_created"), rSet.getInt("modified_by"), rSet.getDate("modification_date"));
				list.add(frenchKeyStageTwo);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static FrenchKeyStageTwo getFrenchKeyStageTwo(long id) {
		FrenchKeyStageTwo frenchKeyStageTwo = null;
		try {
			String query = "select * from tbl_french_key_stage_two where id = "+id;
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				frenchKeyStageTwo = new FrenchKeyStageTwo(rSet.getLong("id"), rSet.getInt("section_id"), 
						rSet.getString("student_id_number"), rSet.getInt("teacher_id"), 
		...
		return frenchKeyStageTwo;
	}
	
	public static List<FrenchKeyStageTwo> getAllKeyStageTwoFrenchEvaluationsForThisSectionAndAcademicYear(int sectionId, int academicYear){
		List<FrenchKeyStageTwo> list = new ArrayList<FrenchKeyStageTwo>();
		FrenchKeyStageTwo frenchKeyStageTwo = null;
		try{
			String query = "select * from tbl_french_key_stage_two where DATE_FORMAT(date_created, '%Y') = "+
